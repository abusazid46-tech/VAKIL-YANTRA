...
if (!TOKEN) {
  console.error('Usage: node deploy_backend.mjs <VERCEL_TOKEN>');
  process.exit(1);
}
const TEAM_ID = 'team_mvhc0BgBg4hwt0yuHSjHgkmi';
const PROJ_ID = 'prj_dqLWFWD3nrdKEW9QUbTKkyj3ubeV';
const SOURCE_DIR = 'C:\\Users\\sonow\\Documents\\Codex\\vakil-yantra\\backend';
...
async function main() {
  console.log('Gathering project files from', SOURCE_DIR);
  const files = getFilesRecursively(SOURCE_DIR);
  console.log(`Prepared ${files.length} files for backend deployment:`);
  files.forEach(f => console.log(` - ${f.file}`));

  console.log('\nTriggering Vercel deployment for vakil-yantra-api...');
  ...
  for (let attempt = 1; attempt <= 45; attempt++) {
    ...
    if (readyState === 'READY') {
      console.log('\n Backend Deployment is READY and live!');
